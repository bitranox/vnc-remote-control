...
**Linux:**
- User config: `~/.config/vnc-remote-control/config.toml`
- App config: `/etc/xdg/vnc-remote-control/config.toml`
...
vnc-remote-control --profile production config

# Override settings at runtime (repeatable)
vnc-remote-control --set lib_log_rich.console_level=DEBUG config
vnc-remote-control --set email.from_address=test@example.com --set email.smtp_hosts='["smtp.example.com:587"]' send-email ...

# Load configuration from an explicit .env file
vnc-remote-control --env-file /etc/myapp/.env config
vnc-remote-control --env-file ./environments/production.env send-notification ...

# Show full traceback for debugging
vnc-remote-control --traceback config-deploy --target user
```
...
vnc-remote-control config

# Output as JSON (useful for scripting)
vnc-remote-control config --format json

# Show specific section only
vnc-remote-control config --section lib_log_rich

# Load configuration for a specific profile
vnc-remote-control config --profile production

# Combine options
vnc-remote-control config --profile staging --format json --section email
```
...
vnc-remote-control config-deploy --target user

# Deploy to system-wide location (requires privileges)
sudo vnc-remote-control config-deploy --target app

# Deploy host-specific configuration
sudo vnc-remote-control config-deploy --target host

# Deploy to multiple locations at once
vnc-remote-control config-deploy --target user --target host

# Overwrite existing configuration
vnc-remote-control config-deploy --target user --force

# Deploy to a specific profile directory
vnc-remote-control config-deploy --target user --profile production

# Deploy production profile and overwrite if exists
vnc-remote-control config-deploy --target user --profile production --force
```
...
sudo -u serviceaccount vnc-remote-control config-deploy --target user

# Deploy with a specific profile
sudo -u serviceaccount vnc-remote-control config-deploy --target user --profile production

# The config will be created at that user's home directory:
# /home/serviceaccount/.config/vnc-remote-control/config.toml
```
...
sudo -u myservice vnc-remote-control config-deploy --target user
...
vnc-remote-control config-deploy --target user --no-permissions

# Override directory mode (octal)
vnc-remote-control config-deploy --target user --dir-mode 750

# Override file mode (octal)
vnc-remote-control config-deploy --target user --file-mode 640

# Both overrides together
vnc-remote-control config-deploy --target user --dir-mode 750 --file-mode 640

# Octal formats: both "750" and "0o750" are accepted
vnc-remote-control config-deploy --target user --dir-mode 0o750
```
...
vnc-remote-control config-generate-examples --destination ./examples

# Overwrite existing example files
vnc-remote-control config-generate-examples --destination ./examples --force

# Generate examples in current directory
vnc-remote-control config-generate-examples --destination .
```
...
vnc-remote-control --set lib_log_rich.console_level=DEBUG --set lib_log_rich.console_format_preset=short config

# Override nested values
vnc-remote-control --set email.attachments.max_size_bytes=10485760 config

# Override with JSON arrays/objects (use single quotes around the value)
vnc-remote-control --set email.smtp_hosts='["smtp.example.com:587"]' config
vnc-remote-control --set email.recipients='["admin@example.com", "ops@example.com"]' config

# Combine with profile
vnc-remote-control --profile production --set lib_log_rich.console_level=DEBUG config
```
...
**Without profile:**
- `~/.config/vnc-remote-control/config.toml`

**With profile `production`:**
- `~/.config/vnc-remote-control/profile/production/config.toml`
...
```bash
LOG_CONSOLE_LEVEL=DEBUG vnc-remote-control hello
LOG_ENABLE_GRAYLOG=true LOG_GRAYLOG_ENDPOINT="logs.example.com:12201" vnc-remote-control hello
```
...
```bash
VNC_REMOTE_CONTROL___LIB_LOG_RICH__CONSOLE_LEVEL=DEBUG vnc-remote-control hello
VNC_REMOTE_CONTROL___EMAIL__SMTP_HOSTS='["smtp.example.com:587"]' vnc-remote-control hello
```
...
vnc-remote-control --env-file /opt/myapp/config/.env config
vnc-remote-control --env-file ./environments/staging.env send-notification ...
```
...
```python
import vnc_remote_control as btcli
